...
import com.example.floatingwidgets.databinding.ViewChronometerBinding

class ChronometerView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, ): LinearLayout(context, attrs) {
    //@JvmOverloads注解指示Kotlin编译器为该函数生成替代默认参数值的重载。如果在xml文件中使用了这个View，就必须要用带attrs的构造函数

    private val binding = ViewChronometerBinding.inflate(LayoutInflater.from(context), this, true)  //父组件是this并绑定，即绑定自己
